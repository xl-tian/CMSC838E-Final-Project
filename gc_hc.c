...
void mark_hot(val_t* addr) {
    ...
    if (alloc_bit_array == 0) {
        bit_array = bit_array_create(100);
        if (bit_array == NULL) {
            printf("Allocating memory for bit array is unsuccessful.\n");
            exit(0);
        }
        alloc_bit_array = 1;
    }

    val_t* raw_addr = (val_t*) (((val_t) addr) & (~7));

    int off_set = raw_addr - from;

    bit_array_set(bit_array, off_set);

    // printf("Ptr Addr: %p\n", addr);
    // printf("Raw Addr: %p\n", raw_addr);
    // printf("Off Set: %i\n", off_set);
    // printf("Bit 1: %d\n", bit_array_test(bit_array, 1));
...
val_t* collect_garbage(val_t* rsp, val_t *rbp, val_t* rbx) {
    ...
    while (rsp < rbp) {
        ...
        if (type >= T_BOX && type <= T_PROC) {
            val_t* raw_addr = (val_t*) (val & (~7));
            int off_set = raw_addr - from;
            
            if (bit_array_test(bit_array, off_set) == 1) {
                rbx = rbx_hot;
                rbx_double = &rbx_hot;
            }
            else {
        ...
        while (top_stk != 0) {
            ...
            process_element(interior, interior_type, curr, rbx, rbx_double);
        }
    ...
    printf("To Space Hot Region:\n");
    print_memory(to, rbx_hot, "heap");
    printf("To Space Cold Region:\n");
    print_memory(old_rbx_cold, rbx_cold, "heap");

    bit_array_zero(bit_array);
    rbx = rbx_hot;
...
void process_element(val_t interior, type_t interior_type, val_t* curr, val_t* rbx, val_t** rbx_double) {
    ...
    if (interior_type >= T_BOX && interior_type <= T_PROC) {
        val_t* raw_addr = (val_t*) (interior & (~7));
        int off_set = raw_addr - from;
