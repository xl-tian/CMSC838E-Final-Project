...
val_t* collect_garbage(val_t* rsp, val_t *rbp, val_t* rbx) {
    ...
    while (curr < rbx) {
        ...
        if (type == T_BOX) {
            ...
            val_t* proc_loc = (val_t*) (interior);
            uint64_t num_args = (uint64_t) *(proc_loc-1);

            // process_element(interior, T_PROC, curr, rbx, &rbx);

            for (int i = 0; i < num_args; ++i) {
                interior = *(curr+i+1);
                val_t interior_type = val_typeof(interior);
                process_element(interior, interior_type, curr+i+1, rbx, &rbx);
            }
